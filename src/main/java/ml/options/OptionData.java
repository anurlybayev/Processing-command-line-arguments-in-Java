...
public class OptionData {
    ...
    OptionData(Prefix prefix, String key, boolean detail, Separator separator,
        ...
        if (value) {
            if (separator == Separator.BLANK) {
                if (detail) {
                    pattern = Pattern.compile(prefix.getName() + key
                            + "((\\w|\\.)+)$");
                } else {
                    pattern = Pattern.compile(prefix.getName() + key + "$");
                }
    ...
    public String getResultValue(int index) {
        ...
        if (index < 0 || index >= getResultCount()) throw
            new IllegalArgumentException(CLASS + ": illegal value for index");
        ...
        if (value) {
            if (valueData == null) {
                throw new IllegalArgumentException(
                        CLASS + ": valueData may not be null");
            }
            ...
            if (detail) {
                if (detailData == null) {
                    throw new IllegalArgumentException(
                            CLASS + ": detailData may not be null");
                }
