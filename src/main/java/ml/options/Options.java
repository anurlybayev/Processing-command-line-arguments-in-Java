...
public class Options {
    ...
    public Options(String args[], Prefix prefix,
        ...
        if (args == null)
            throw new IllegalArgumentException(
                CLASS + ": args may not be null");

        if (prefix == null)
            throw new IllegalArgumentException(
                CLASS + ": prefix may not be null");

        if (defaultMultiplicity == null)
            throw new IllegalArgumentException(
                CLASS + ": defaultMultiplicity may not be null");

        if (defMinData < 0)
            throw new IllegalArgumentException(
                CLASS + ": defMinData must be >= 0");

        if (defMaxData < defMinData)
            throw new IllegalArgumentException(
                CLASS + ": defMaxData must be >= defMinData");
    ...
    public OptionSet addSet(String setName, int minData, int maxData) {
        ...
        OptionSet os = new OptionSet(
                prefix, defaultMultiplicity, setName, minData, maxData);
    ...
    public boolean check(String setName, boolean ignoreUnmatched,
        ...
        if (optionSets.get(setName) == null)
            throw new IllegalArgumentException(
                CLASS + ": Unknown OptionSet: " + setName);
        ...
        if (options.size() == 0) { // No options have been defined at all
            if (arguments.length == 0) { // No arguments have been given: in
                ...
                checkErrors.append("No options have been defined, ");
                checkErrors.append("nothing to check\n");
                return false;
            }
        } else if (arguments.length == 0) { // Options have been defined, but no
            // arguments given
            checkErrors.append("Options have been defined, but no arguments ");
            checkErrors.append("have been given; nothing to check\n");
            return false;
        }
        ...
        while (true) {
                ...
                if (m.lookingAt()) {
                    ...
                    if (optionData.useValue()) {
                        if (optionData.useDetail()) {
                            detail = m.group(1);
                            // required for correct Matcher.group access below
                            offset = 2;
                        }
                        ...
                        if (optionData.getSeparator() == Separator.BLANK) {
                            ...
                            if (ipos + 1 == arguments.length) {
                                // Error
                                checkErrors.append("At end of arguments - ");
                                checkErrors.append("no value found ");
                                checkErrors.append("following argument ");
                    ...
                    // Store the result
                    if (add) optionData.addResult(value, detail);
            ...
            if (ipos >= arguments.length) break;
        ...
        if (requireDataLast) {
            if (first + data.size() != arguments.length) {
                checkErrors
                        .append("Invalid data specification: data arguments ");
                checkErrors
                        .append("are not the last ones on the command line\n");
                return false;
            }
        }
    ...
    public void addOptionAllSets(String key, Multiplicity multiplicity) {
        for (String setName : optionSets.keySet()) {
            optionSets.get(setName).addOption(
                    key, false, Separator.NONE, false, multiplicity);
        }
    }
    ...
    public void addOptionAllSets(String key, Separator separator) {
        for (String setName : optionSets.keySet()) {
            optionSets.get(setName).addOption(
                    key, false, separator, true, defaultMultiplicity);
        }
    }
    ...
    public void addOptionAllSets(String key, Separator separator,
        ...
        for (String setName : optionSets.keySet()) {
            optionSets.get(setName).addOption(
                    key, false, separator, true, multiplicity);
        }
    }
    ...
    public void addOptionAllSets(String key, boolean details,
        ...
        for (String setName : optionSets.keySet()) {
            optionSets.get(setName).addOption(
                    key, details, separator, true, defaultMultiplicity);
        }
    }
    ...
    public void addOptionAllSets(String key, boolean details,
        ...
        for (String setName : optionSets.keySet()) {
            optionSets.get(setName).addOption(
                    key, details, separator, true, multiplicity);
        }
    }
